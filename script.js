...
*/
(function () {
if (!window.verify_description) {
	document.getElementsByTagName('body')[0].classList.add('without-timegrip-helper');
	return false;
}
...
function timeGripHelper() {
  var $mainWrapper = $j('<div class="main-wrapper"></div>'),
      ...
      init = function () {
		...
		aEqRegExp = eval(new RegExp('\\' + aEq,'gim'));
		oEqRegExp = eval(new RegExp('\\' + oEq,'gim'));
		aeEqRegExp = eval(new RegExp('\\' + aeEq ,'gim'));
      ...
      searchProjectByName = function (value) {
        ...
        if (!$projectSelect.length) {
        	$j('#regITM_set1').hide();
	        $j('#regITM_set2').show();
	        $projectSelect = $projectsSelectOnItemRecord;
        }
        $projectSelect.find('option').each(function () {
          var $this = $j(this),
              id = $this.attr('value').toLowerCase(),
              rating = 0,
              text = $this.text().toLowerCase(),
              enEquivalent = text.replace(aEqRegExp,'aa').replace(oEqRegExp,'o').replace(aeEqRegExp,'ae');

	         $.each(value, function (ind, item) {
		          if (id.indexOf(item) > -1 || text.indexOf(item) > -1 || enEquivalent.indexOf(item) > -1) {
		          	rating++;
		          }
	         });
      ...
      renderYourProjects = function () {
      	...
      	$yourProjects.empty();
      	$yourProjectsSelect.find('optgroup').each(function () {
      		var $opt = $j(this),
      			optText = $opt.attr('label');

      		html += '<li class="timegrip-your-proj-group">' + optText + '</li>';
	      	...
	      	$opt.find('option').each(function () {
		         ...
		         html += '<li><a class="timegrip-your-proj-item" data-project="' + projectID +'" href="#">' + text + '</a></li>';
	      	});
      	})
      	$yourProjects.append(html + '</ul>');
      },
      renderProjectRes = function (res) {
      ...
      addToFavorites = function () {
      	var $select = $yourProjectsSelect.is(':visible') ? $yourProjectsSelect : $projectSelect;
      	var projectOpt = $select.find('option:selected'),
      		activityOpt = $activitySelect.find('option:selected'),
      		obj = {
	      		activityID: activityOpt.attr('value'),
	      		projectID: projectOpt.attr('value'),
	      		project: projectOpt.text(),
	      		activity: activityOpt.text()
	      	};
	      favoritesArray.push(obj);
	      window.localStorage.setItem('favorites', JSON.stringify(favoritesArray));
	      renderFavorites();
      },
      getFavories = function () {
      	favoritesArray = [];
      	$favoriteHolder.find('.timegrip-fav-item').each(function () {
      		var $this = $j(this);
      		var obj = {
	      		activityID: $this.data('activity'),
	      		projectID: $this.data('project'),
	      		project: $this.data('projectText'),
	      		activity: $this.data('activityText')
	      	};
	      	favoritesArray.push(obj)
      	});
	    window.localStorage.setItem('favorites', JSON.stringify(favoritesArray));
      ...
      attachEvents = function () {
        ...
        $projectSearchRes.on('click', '.timegrip-proj-res-item', function (e) {
          e.preventDefault();
          setProject($j(this).text());
          $j(this).parent().siblings().removeClass('active');
          $j(this).parent().addClass('active');
        });
        $activityRes.on('click', '.timegrip-activity-res-item', function (e) {
          e.preventDefault();
